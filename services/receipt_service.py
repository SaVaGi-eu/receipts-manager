...
Moved to services/ package as part of RM-115 folder structure refactor.
"""
...
def _sanitize_log(text: object, max_length: int = 200) -> str:
    """
...
class ReceiptService:
    ...
    def export_csv(self) -> str:
        ...
        for item in data.get("items", []):
            ...
            ew = item.get("extended_warranty") or {}
            writer.writerow(
                [
                    item["id"],
                    ...
                    item.get("guarantee_unit", "months"),
    ...
    def create_item(self, receipt_group_id: str, item_data: dict) -> dict:
        """Create a new item in an existing receipt group (RM-123 / RM-110)."""
        with self._lock:
            data = self.load()
            receipt = next((r for r in data["receipts"] if r["receipt_group_id"] == receipt_group_id), None)
            if not receipt:
                raise KeyError("Receipt group not found")
            item_id = int(data.get("next_id", 1))
            rel_path = receipt.get("receipt_relative_path", "")
            duration = int(item_data.get("guarantee_duration") or 0)
            unit = item_data.get("guarantee_unit", "months")
            item = {
                "id": item_id,
                ...
                "project": item_data.get("project", "N/A"),
                "guarantee_duration": duration,
                "guarantee_unit": unit,
                "guarantee_end_date": calculate_guarantee_end_date(receipt["purchase_date"], duration, unit),
                "price": item_data.get("price"),
                "extended_warranty": item_data.get("extended_warranty"),
                "receipt_relative_path": rel_path,
            }
            data.setdefault("items", []).append(item)
            data["next_id"] = item_id + 1
    ...
    def get_settings(self) -> dict:
        """Return app-level settings stored in data.json."""
        with self._lock:
            data = self.load()
        defaults = {
            "currency": "EUR",
